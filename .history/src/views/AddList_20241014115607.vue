  ...
  <n-message-provider>
    <div class="box">
      <div class="background">
        ...
        <div class="mask" v-if="mergePaymentOn">
          <div class="background">
            <div class="eachPayment">
              ...
              <n-input ref="firstInput" v-model:value="inputValues[selectedOptions[0]]" type="text"
                :placeholder="`${selectedOptions[0]}支出金額`" @focus="setFocusInput('firstInput')" />
            </div>
            <div class="eachPayment">
              <p>{{ selectedOptions[1] + '：' }}</p>
              <n-input ref="secondInput" v-model:value="inputValues[selectedOptions[1]]" type="text"
                :placeholder="`${selectedOptions[1]}支出金額`" @focus="setFocusInput('secondInput')" />
            </div>
            <div class="result">
        ...
        <div class="amount">
          <div class="screen">
            <n-input v-model:value="screenValue" type="text" placeholder=" " :disabled="mergePaymentFont" />
...
const paymentOption = ref(['現金', '信用卡', '金融卡', '悠遊卡'])
const selectedOptions = ref<string[]>([])
const mergePaymentFont = ref(false)
const mergePaymentOn = ref(false)
const inputValues = ref<{ [key: string]: string }>({ '現金': '', '信用卡': '' })
const focusedOnWhichone = ref<string | null>(null)
...
function setFocusInput(inputRef: string) {
  focusedOnWhichone.value = inputRef
}

function num(number: string) {
  if (focusedOnWhichone.value) {
    console.log(focusedOnWhichone.value)
    inputValues.value[focusedOnWhichone.value] += number
  } else {
...
function point() {
  if (mergePaymentOn.value) {
    if (!inputValues.value.includes('.')) {
      inputValues.value += '.'
    }
  } else {
...
screenValue.value = inputValues.value + inputValues.value
