<template>
  <div class="box">
    <div class="background">
      ...
      <div class="content">
        <div class=calendar>
          <n-calendar #="{ month, date }" v-model:value="value" @update:value="handleUpdateValue">
...
const calculateDailyInput = (month: number, date:number) => {
  const calendarList = lists.value.filter((item: any) => {new Date(item.date)})
  console.log('列表：', calendarList)
  const calendarListInput = calendarList.filter((item: any) => item.type === '收入')
  console.log('收入：', calendarListInput)
...
onMounted(() => {
  ...
  input.value = calculateDailyInput(month, date)
})

watch(lists, (newValue) => {
  input.value = calculateDailyInput(month, date)
  console.log('naive-ui裡面的日期', value.value)
})
</script>
