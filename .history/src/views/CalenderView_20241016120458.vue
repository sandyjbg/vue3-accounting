      ...
      <div class="background">
        ...
        <div class="content">
          ...
          <div class="dailyDetail">
            <div class="eachType" v-for="(items, category) in categorizedList">
              <div class="typeTitle" :style="{ backgroundColor: items[0]?.color; }">
                <n-icon :component="items[0]?.icon" />
                <p class="type">{{ category }}</p>
              </div>
              <p class="totalAmount" :style="{ color: items[0]?.color; }">290元</p>
              <div class="repeatTypeContent">
                <div class="detailContent" v-for="(item, index) in items">
...
function pressed() {
  ...
  if (sortDatebyList.value[valueDate]) {
    categorizedList.value = sortDatebyList.value[valueDate]
  } else {
...
const sortDatebyList = computed(() => {
  return lists.value.reduce((sortList: any, list: any) => {
    ...
    if (!sortList[list.date][list.category]) {
      sortList[list.date][list.category] = []
    }

    sortList[list.date][list.category].push(list)
    return sortList
  }, {})
})
...
onMounted(() => {
  ...
  console.log('aaa', sortDatebyList)
})
