...
// 在同一天項目之下歸類出相同種類的支出收入
Object.keys(sortDatebyList.value).forEach(date => {
  const items = sortDatebyList.value[date]

  const categorizedList = items.reduce((acc: any, thing: any) => {
    if (!acc[thing.category]){
    acc[thing.category] = []
  }
