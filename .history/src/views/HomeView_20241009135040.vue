        ...
        <div class="dayDetail" v-for="(items, date) in sortListbyTime" :key="date">
          ...
          <div class="dayList">
            <div class="eachList" v-for="list in [...items.income, ...items.expense]"
              ...
              <div class="number">
                {{ list.type === '收入' ? '+' : '-' }}{{ Number(list.amount) }}
              </div>
            </div>
          </div>
        </div>
...
watch(lists, (newValue) => {
  ...
  console.log('Data stored in localStorage:', localStorage.getItem('lists'))
}, { deep: true })
...
// 检查是否成功存储
console.log('Manually stored test data:', localStorage.getItem('test'))
})
