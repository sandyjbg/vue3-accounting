      ...
      <div class="background">
        ...
        <div class="content">
          <div class="dayDetail" v-for="(items, date) in sortListbyTime">
            ...
            <div class="dayList">
              <draggable v-model="items.expense" @dragend="onDragChange(items, date)" :item-key="id">
...
const sortListbyTime = computed(() => {
  if (!Array.isArray(lists.value) || lists.value.length === 0) {
    return reactive({}); // 返回一个响应式的空对象
  }

  // 使用 reduce 来创建 timeList，并确保它是 reactive 的
  const timeList = reactive({});

  lists.value.forEach((list: any) => {
    // 初始化 income 和 expense 为响应式的数组
  ...
  return timeList; // 返回响应式的 timeList
});
