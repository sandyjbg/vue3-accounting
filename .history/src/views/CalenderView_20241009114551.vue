...
const calculateInput = (year: number, month: number, date: number) => {
  ...
  const calculate = lists.value.filter((item: any) => (item.date).isDate().split('T')[0] === equalDate)
...
const handleUpdateValue = (newValue: number, year:number, month:number, date:number) => {
  ...
  calculateInput(selectedYear.value, selectedMonth.value, selectedDate.value)
