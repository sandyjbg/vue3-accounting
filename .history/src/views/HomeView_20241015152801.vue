              ...
              <div class="biggerEachList" v-for="(list, index) in [...items.income, ...items.expense]">
                <div
                  ...
                  <div class="project">
                    ...
                    <n-icon :component="list.icon" :style="{ backgroundColor: list.color , marginLeft: (list.merge === false && mergeStart === true) ? '20px' : '' }" />
                    <p>{{ list.category }}</p>
...
function closeAllFunction(index:number){
  ...
  lists.value.forEach(item => item.merging = false)
}
