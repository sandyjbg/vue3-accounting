      ...
      <div class="background">
        ...
        <div class="content">
          <div class="dayDetail" v-for="(items, date) in sortListbyTime">
            ...
            <div class="dayList">
              <draggable v-model="items.expense" @change="onDragChange(items, date)" :key="items.id">
                <template #item="{ element }" :key="items.id">
                  <div class="biggerEachList">
                    <div
                      :class="['eachList', { more: !moreFunction, input: element.type === '收入', output: element.type !== '收入' }]">
                      <div class="project">
                        <n-checkbox v-if="element.merging" />
                        <n-icon :component="element.icon"
                          :style="{ backgroundColor: element.color, marginLeft: (element.merging === false && mergeStart === true) ? '23px' : '' }" />
                        <p>{{ element.category }}</p>
                        <p v-if="element.description">-- {{ element.description }}</p>
                      </div>
                      <div class="number" v-if="element.moreFunction">
                        <div class="payment">
                          <p v-for="pay in element.payment">{{ pay }}</p>
                        </div>
                        <p class="num">{{ element.type === '收入' ? '+' : '-' }}{{ Number(element.amount) }}</p>
                      </div>
                    </div>
                  </div>
                </template>
...
const onDragChange = async (items: any, date: any) => {
  console.log(`在 ${date} 中的數據被更新: `, items);
  items.expense = [...items.expense]; // 确保对整个数组重新赋值以触发响应
  await nextTick(); // 确保 Vue 下一次更新之后 DOM 已经重新渲染
  items.expense = [...items.expense]
...
const sortListbyTime = computed(() => {
  ...
  return lists.value.reduce((timeList: any, list: any) => {
    ...
    if (!timeList[list.date]) {
      timeList[list.date] = { income: [], expense: ref([]) }
    }
    if (list.type === '收入') {
      timeList[list.date].income.push(list)
    } else if (list.type === '支出') {
      timeList[list.date].expense.value.push(list)
    }
...
onMounted(() => {
  ...
  lists.value = accountingStore.accountingData.map((item: any) => {
    return {
      ...item,
      expense: ref(item.expense) // 确保 expense 是响应式的
    };
  });
