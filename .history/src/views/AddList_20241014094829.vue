<template>
  ...
  <div class="background">
    ...
    <div class="eachPayment" v-for="(option, index) in selectedOptions" :key="index">
      <p>{{ option + '：' }}</p>
      <n-input v-model:value="inputValues[index]" type="text" :placeholder="`${option}支出金額`" />
    </div>
    <div class="result">
      <div class="confirm">確定</div>
      <div class="cancel" @click="closeMerge()">取消</div>
    </div>
  </div>
</div>
      ...
      <div class="amount">
        <div class="screen">
          ...
          <div v-if="mergePaymentFont" class="warning" @click="showMergePayment = true">
...
const inputValues = ref(Array(selectedOptions.length).fill(''))
...
.box {
  ...
  .background {
    ...
    border-radius: 5px;
    // background-color: rgb(255, 244, 230);
    box-shadow: 0px 0px 2px 2px rgba(0, 0, 0, 0.356);
