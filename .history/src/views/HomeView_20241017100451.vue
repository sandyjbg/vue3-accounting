  ...
  <n-message-provider>
    <div class="box">
      <div class="background">
        ...
        <div class="content">
          <div class="dayDetail" v-for="(items, date) in doneSortingLists" :key="date">
            <div class="dayTotal">
              <div class="date">
                <p>{{ date }}</p>
              ...
              <div class="smallTotal">
                <div class="smallDailyTotal">
                  <p>日增減</p>
                  <p>{{ dailyTotalAmount(items) }} 元</p>
                </div>
                <div class="smallInput">
                  <p>收入</p>
                  <p>{{ dailyTotalInput(items) }} 元</p>
                </div>
                <div class="smallOutput">
                  <p>支出</p>
                  <p>{{ dailyTotalOutput(items) }} 元</p>
                </div>
              </div>
            </div>
            <div class="dayList">
              <div class="biggerEachList" v-for="(list, index) in items">
                <div
...
// 計算日支出 / 收入 / 日增減
function dailyTotalInput(wholeList: any) {
  return wholeList.filter((item: any) => item.type = '收入').reduce((total: number, item: any) => total + Number(item.amount), 0)
}
function dailyTotalOutput(wholeList: any) {
  return wholeList.filter((item: any) => item.type = '支出').reduce((total: number, item: any) => total + Number(item.amount), 0)
}
function dailyTotalAmount(wholeList:any) {
  const income = wholeList.filter((item: any) => item.type = '收入').reduce((total: number, item: any) => total + Number(item.amount), 0)
  const expense = wholeList.filter((item: any) => item.type = '支出').reduce((total: number, item: any) => total + Number(item.amount), 0)
