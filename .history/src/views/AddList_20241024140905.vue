<template>
  <n-message-provider>
    <div class="box">
      <div class="background">
        ...
        <div class="mask" v-if="mergePaymentOn">
          <div class="background">
            <div class="eachPayment" v-for="option in selectedOptions" :key="option">
              <p>{{ option + '：' }}</p>
              <n-input :ref="option" v-model:value="inputValues[option]" type="text" :placeholder="`請輸入${option}支出金額`"
                @focus="setFocusInput(option)" />
            </div>
            <div class="result">
        ...
        <div class="amount">
          ...
          <div class="account">
            ...
            <div class="paymentList">
              ...
              <p @click="showLentOrReimburse = true"><n-icon :component="ReceiptFilled" />報銷</p>
              <p @click="showLentOrReimburse = true"><n-icon :component="FrontHandRound" />借用</p>
    ...
    <PaymentChoose v-if="showPaymentChoose" :paymentList="paymentOption" :paymentConfirm="paymentConfirm"
      ...
      <LentorReimburse v-if="showLentOrReimburse" />
  </n-message-provider>
</template>
...
import LentorReimburse from '../components/addlist/LentorReimburse.vue'
...
const showLentOrReimburse = ref(false)
...
// 點出混合支付又點回單一支付的時候字體消失
watch(selectedOptions, (newValue) => {
  if (newValue.length === 1 && newValue[0] !== '混合') {
    mergePaymentFont.value = false
  }
})
...
function setFocusInput(inputRef: string) {
  ...
  if (!inputValues.value[inputRef]) {
    inputValues.value[inputRef] = ''
  }
}
...
function num(number: string) {
  if (focusedWhichOne.value) {
    // 确保该支付选项的值已经初始化
    if (!inputValues.value[focusedWhichOne.value]) {
