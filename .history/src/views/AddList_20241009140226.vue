    ...
    <div class="background">
      ...
      <div class="category">
        <div class="eachCategory" v-for="(category, idx) in filterCategories" :key="category.name"
          ...
          <n-icon :component="categoryIcon.value" :style="{ backgroundColor: category.color }" />
          <p>{{ category.name }}</p>
        </div>
      </div>
...
const categoryIcon = ref<typeof FreeBreakfastRound>(FreeBreakfastRound)
...
nextTick(() => {
  categoryIcon.value = ref<typeof FreeBreakfastRound>(FreeBreakfastRound)
})
...
function vmodelicon(name: string, icon: Component, color: string) {
  ...
  categoryIcon.value = icon as typeof FreeBreakfastRound
