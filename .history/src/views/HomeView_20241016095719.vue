              ...
              <div class="biggerEachList" v-for="(list, index) in [...items.income, ...items.expense]">
                <div
                  ...
                  <div class="project">
                    ...
                    <n-icon :component="list.icon" :style="{ backgroundColor: list.color , marginLeft: (list.merging === false && mergeStart === true) ? '23px' : '' }" />
                    <p>{{ list.category }}</p>
...
const sortListbyTime = computed(() => {
  ...
  return sortedLists.reduce((timeList: any, list: any) => {
    ...
    if (!timeList[date]) {
      timeList[date] = { income: [], expense: [] }
    }
    if (list.type === '收入') {
      timeList[date].income.push(list)
    } else if (list.type === '支出') {
      timeList[date].expense.push(list)
    }
