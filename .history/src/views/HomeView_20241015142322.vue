          ...
          <div class="dayList">
            <div class="biggerEachList" v-for="(list, index) in [...items.income, ...items.expense]">
              ...
              <div v-else class="list">
                <n-icon class="back" :component="ArrowForwardIosFilled" @click="list.moreFunction = true" />
...
import { DensityMediumTwotone, ArrowLeftRound, ArrowRightRound, ArrowForwardIosFilled, ContentCopyFilled, DeleteFilled, EditFilled } from '@vicons/material'
...
function listsCopy(index:number){
  const newList = JSON.parse(JSON.stringify(lists.value[index]));
  newList.id = lists.value.length + 1;
  lists.value.push(newList);
  console.log('原始項目:', lists.value[index]);
  console.log('深拷貝前後對比:', JSON.stringify(lists.value[index]), JSON.parse(JSON.stringify(lists.value[index])));
}
