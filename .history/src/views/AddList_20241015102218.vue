<template>
  <n-message-provider>
    ...
    <input type="text" v-model="address" @keyup.enter="searchLocation" placeholder="請輸入地址" class="address-input" />
    <button @click="searchLocation">搜尋</button>
...
const focusedWhichOne = ref<string | null>(null)
  const map = ref(null);
const address = ref('');  // 使用者輸入的地址
let geocoder;  // 用於地理編碼
let mapInstance;  // Google 地圖實例
  ...
  if (address.value) {
    geocoder.geocode({ address: address.value }, (results, status) => {
      if (status === 'OK' && results[0]) {
        ...
        mapInstance.setCenter(location);  // 將地圖中心設置為該地點
        new google.maps.Marker({
          map: mapInstance,
          position: location,
        });
      } else {
        alert('找不到該地址，請確認後重新輸入');
      }
    });
  }
};
...
onMounted(() => {
  ...
  mapInstance = new google.maps.Map(map.value, {
    center: { lat: 25.033964, lng: 121.564468 },  // 初始位置：台北 101
    zoom: 13,
  });
...
function commit() {
  const dataToSubmit = {
    ...
    payment: selectedOptions.value.length >= 2 ? inputValues.value : selectedOptions.value
  }
