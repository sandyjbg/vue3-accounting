  ...
  <n-message-provider>
    <div class="box">
      <div class="background">
        ...
        <div class="category">
          <draggable v-model="filterCategories" @end="onEnd" item-key="name">
            <template #item="{ element }">
          <div class="eachCategory"
            @click="element.name === '添加分類' ? (showAddCategory = true, isEditMode = false) : vmodelicon(element.name, element.icon, element.color)"
            @contextmenu.prevent="element.name === '添加分類' ? () => { } : dropDownMenu($event, element)">
            <n-icon :component="element.icon" :style="{ backgroundColor: element.color }" />
            <p>{{ element.name }}</p>
          </div>
        </template>
      </draggable>
